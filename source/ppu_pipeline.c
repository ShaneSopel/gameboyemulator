...
bool window_visible()
{
    //return is the window on the visible screen?
...
u32 fetch_sprite_pixels(int bit, u32 color, u8 bg_color)
{
    for (int i=0; i<ppu_get_context()->fetched_entry_count; i++)
    {
        ...
        u8 hi = !!(ppu_get_context()->pfc.fetch_entry_data[i * 2] &
            (1 << bit));

        u8 lo = !!(ppu_get_context()->pfc.fetch_entry_data[(i * 2 ) + 1 ] &
            (1 << bit)) << 1;

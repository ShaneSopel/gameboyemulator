...
{
    // instruction sets for each row of the opcode
    // 0x0x to 0xFx
    ...
    [0xE7] = {IN_RST, AM_IMP, RT_NONE, RT_NONE, 0x20},
...
void inst_to_str(cpu_context *con, char *str) {
    instruction *inst = con->cur_inst;
    ...
    switch(inst->mode) {
        ...
        case AM_R_A16:
            sprintf(str, "%s %s,$%04X", inst_name(inst->type), 
                rt_lookup[inst->reg_1], con->fetch_data);
            return;
        ...
        case AM_R_A8:
            sprintf(str, "%s %s,$%02X", inst_name(inst->type), 
                rt_lookup[inst->reg_1], con->fetch_data & 0xFF);
            return;
        ...
        case AM_A8_R:
            sprintf(str, "%s $%02X,%s", inst_name(inst->type), 
                bus_read(con->regs.pc - 1), rt_lookup[inst->reg_2]);
        ...
        case AM_A16_R:
            sprintf(str, "%s ($%04X),%s", inst_name(inst->type), 
                con->fetch_data, rt_lookup[inst->reg_2]);
            return;
